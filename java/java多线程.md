...
* 线程的优先级数为1~10
    ...
    * **高优先级的线程进入就绪状态会使低优先级的线程暂停，立即让出资源**
    * **使用yield()可以让出当前的资源给其他线程执行，但仅限于同优先级的**
...
* 线程体即为Thread的run()方法，并由start()方法所启动
    * 注意：调用start()只是将线程置为就绪状态，线程不会立即运行，二是等待CPU去调度

* 只有主线程完毕或休眠后，子线程才有机会能够运行

* 一般来说，同优先级的子线程会被CPU按照start的顺序依次调用，**只有一个子线程完毕或休眠，下一个子线程才有机会运行**（或者由线程调度器决定）
...
* Thread A.setPriority(int p)：为线程A设置优先级

* Thread A.setDaemon(boolean b)：是否将线程A设置为后台守护进程，该方法必须在start()前调用

* boolean Thread A.isAlive()：测试线程A是否处于活动状态，即线程被启动且没有死亡
